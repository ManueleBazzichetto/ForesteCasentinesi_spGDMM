...
too_close_to_drop <- as.data.frame(too_close_to_drop)

#drop duplicates
too_close_to_drop <- too_close_to_drop[!duplicated(too_close_to_drop), ]

#check if some entries occur more than once - these plots will be dropped first
too_close_to_drop[duplicated(too_close_to_drop$row), 'row'] #0
too_close_to_drop[duplicated(too_close_to_drop$col), 'col'] #0

#collect ids to drop -> only needed in case of duplicates plots
#too_close_duply <- c(too_close_to_drop[duplicated(too_close_to_drop$row), 'row'], too_close_to_drop[duplicated(too_close_to_drop$col), 'col'])
#drop duplicated
#too_close_duply <- too_close_duply[!duplicated(too_close_duply)]
#drop duply rows
#too_close_to_drop <- too_close_to_drop[!too_close_to_drop$row %in% c(), ]
#drop duply cols
#too_close_to_drop <- too_close_to_drop[!too_close_to_drop$col %in% c(), ]

#choose plots to drop

#drop 1) smaller (in terms of plot size) plots and 2) older
#note that in case of same plot size and year the first plot is selected - no need of set.seed
too_close_ids <- sapply(1:nrow(too_close_to_drop), function(i) {
  to_filter <- Forcas_env[unlist(too_close_to_drop[i, ]), c('PlotObservationID', 'Plot_size', 'Year')]
  if(nrow(to_filter) > 2) stop("More than to plots to compare")
  if(isTRUE(to_filter[1, 'Plot_size'] != to_filter[2, 'Plot_size'])) {
    id_to_drop <- to_filter[which.min(to_filter[['Plot_size']]), 'PlotObservationID']
    return(id_to_drop)
  } else {
    id_to_drop <- to_filter[which.min(to_filter[['Year']]), 'PlotObservationID']
    return(id_to_drop)
  }
  })

#add id of duply plots if needed
#length(too_close_ids) plots are eventually dropped
#too_close_ids <- c(too_close_ids, Forcas_env[too_close_duply, 'PlotObservationID'])

#check duplicates
sum(duplicated(too_close_ids)) #0

rm(too_close_duply, too_close_to_drop)

#drop the plots from Forcas_env
...
#check
which((geo_distmat_preliminary + diag(1, nrow = nrow(geo_distmat_preliminary))) < min_dist, arr.ind = T) #ok
